...
grep -q '"$RAW_NAME"' "$BUILD_OVA"
grep -q "qemu-img convert -p -O vpc -o subformat=fixed,force_size=on" "$BUILD_OVA"
...
grep -Fq '.format == "vpc" and .["virtual-size"] >= 3221225472' "$BUILD_OVA"
grep -q 'stat -c%s release/net-relay-alpine-x86_64.vhd' "$BUILD_OVA"
