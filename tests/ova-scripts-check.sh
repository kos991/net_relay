...
grep -q 'MIN_ROOT_FREE_MIB: "256"' "$BUILD_OVA"
...
grep -q "LIBGUESTFS_BACKEND: direct" "$BUILD_OVA"
grep -q 'sudo -E virt-df --csv -a "release/${RAW_NAME}"' "$BUILD_OVA"
grep -q 'awk -F, '\''$2 == "/dev/sda1" {print int($5 / 1024)}'\''' "$BUILD_OVA"
...
grep -q 'Error: / needs at least' "$BUILD_OVA"
