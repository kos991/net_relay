...
grep -q "NetBird Relay OVA 首次初始化向导" "$FIRSTBOOT"
grep -q "首次初始化步骤" "$FIRSTBOOT"
grep -q "\\[1/5\\] 环境预检" "$FIRSTBOOT"
grep -q "\\[2/5\\] 根分区扩容" "$FIRSTBOOT"
grep -q "\\[3/5\\] 网络自检" "$FIRSTBOOT"
grep -q "\\[4/5\\] ZRAM 和内核优化" "$FIRSTBOOT"
grep -q "\\[5/5\\] Relay 配置向导" "$FIRSTBOOT"
grep -q "输入 rels 可再次打开管理菜单" "$FIRSTBOOT"
...
grep -q "输入 rels 可再次打开管理菜单" "$FIRSTBOOT"
grep -q "netbird-relay 二进制不存在" "$FIRSTBOOT"
...
grep -q "首次登录需要先修改 root 密码" "$ROOT_PROFILE"
grep -q "expire: true" "$LOGIN_CFG"
grep -q "LANG=zh_CN.UTF-8" "$LOCALE_PROFILE"
grep -q "LC_ALL=zh_CN.UTF-8" "$LOCALE_PROFILE"
grep -q "MUSL_LOCPATH=/usr/share/i18n/locales/musl" "$LOCALE_PROFILE"
...
grep -q "musl-locales" "$BUILD_OVA"
grep -q "musl-locales-lang" "$BUILD_OVA"
grep -q "font-noto-cjk" "$BUILD_OVA"
grep -q "fontconfig" "$BUILD_OVA"
